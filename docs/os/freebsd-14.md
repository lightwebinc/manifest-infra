...
```bash
service shard_manifest status
tail -n 50 /var/log/shard_manifest.log
...
is restarted via the `Restart shard-manifest` handler.

## Notes

- The rc.d script reads the rendered config at
  `/usr/local/etc/shard-manifest.conf` and exports each variable
  before invoking `/usr/sbin/daemon` to fork the binary.
- The pf anchor is loaded from `/etc/pf.anchors/shard-manifest`
